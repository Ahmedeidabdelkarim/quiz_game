...
function checkAnswer() {
  ...
  //Resets the selected option (unchecks it) for future selections.
  selectedOption.checked = false;
  currentQuestion++;
  // If there are more questions remaining, it calls showQuestion() to display the next one.
  if (currentQuestion < questions.length) {
    showQuestion();
  } else {
    // Show the result and Stops the timer by clearing it using clearInterval().
...
function startTimer(durationInMinutes, display) {
  ...
  intervalId = setInterval(function () {
    //Calculates remaining minutes and seconds from total seconds left in timer.
    minutes = parseInt(timer / 60, 10);
    seconds = parseInt(timer % 60, 10);

    //Formats minutes and seconds to always display two digits (e.g., "01" instead of "1").
